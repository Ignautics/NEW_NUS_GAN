...
def imsave(images, ori, size, path):#add contrast(target image)
  ...
  concat = np.zeros((image.shape[0], image.shape[1]*2))
...
def visualize(sess, dcgan, config, option):
  ...
  samples, d_loss, g_loss = sess.run(
          ...
          feed_dict={
              dcgan.inputs: dcgan.sample_inputs,
              dcgan.z: sample_z,
              dcgan.y: dcgan.sample_y,
              dcgan.c2: dcgan.sample_c2
          }
  )
  save_images(samples, dcgan.sample_inputs, image_manifold_size(samples.shape[0]), './samples/test_arange.png')
  print("test finish")
